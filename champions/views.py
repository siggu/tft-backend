...
class Set11Champions(APIView):
    def get(self, request):
        champions = Set11Champion.objects.all()
        serializer = serializers.Set11ChampionDetailSerializer(
            champions,
            many=True,
        )
    ...
    def post(self, request):
        ...
        for champion_data in champion_data_list:
            ...
            # serializer를 초기화합니다.
            serializer = serializers.Set11ChampionDetailSerializer(data=champion_data_copy)

            if serializer.is_valid():
                champion_obj = serializer.save()
                champion_obj_serializer = serializers.Set11ChampionDetailSerializer(
                    champion_obj
                )
...
class Set11ChampionDetail(APIView):
    def get_object(self, key):
        try:
            return Set11Champion.objects.get(key=key)
        except Set11Champion.DoesNotExist:
            raise NotFound
...
class Set12Champions(APIView):
    ...
    def post(self, request):
        ...
        for champion_data in champion_data_list:
            ...
            # serializer를 초기화합니다.
            serializer = serializers.Set12ChampionDetailSerializer(data=champion_data_copy)

            if serializer.is_valid():
                champion_obj = serializer.save()
                champion_obj_serializer = serializers.Set12ChampionDetailSerializer(
                    champion_obj
                )
                response_data.append(champion_obj_serializer.data)
            else:
...
class Set12ChampionDetail(APIView):
    def get_object(self, key):
        try:
            return Set12Champion.objects.get(key=key)
        except Set12Champion.DoesNotExist:
            raise NotFound
