...
# 챌린저 상위 30명의 플레이어 정보 수집
top_players = challenger_df.head(SEARCH_SCALE)

# 상위 30명의 플레이어들의 매치 ID 수집
all_match_ids = set()
...
# 데이터 저장
participants_df.to_json("participants_data.json", orient="records")

# 데이터 로드
participants_df = pd.read_json("participants_data.json")
...
# 챔피언별 아이템 사용 횟수를 JSON으로 저장
with open('champion_item_usage.json', 'w') as f:
    json.dump(champion_item_usage, f, indent=4)
...
# 클러스터 별 챔피언 사용 빈도 시각화
plt.figure(figsize=(24, 18))
sns.heatmap(clustered_units_df.T, cmap='coolwarm', annot=True)
plt.xlabel('Clusters')
plt.ylabel('Champions')
plt.title('Champion Usage in Clusters')
plt.show()
